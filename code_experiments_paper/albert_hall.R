...
group_assignment = c(sapply(1:PERIOD_FOR_PREDICTING,
                            function(x){paste0(x)}))
df = pivot_wider(future_prevalence_df %>% 
...
mult = sapply(1:28, function(i){ rnorm(n =B,infectiousness_all$infectiousness_event[i], infectiousness_all$infectiousness_event_sd[i])})
mult[mult<=0] =0
...
#  }))
#n_susc = sapply(1:B, function(b){(N_TOT-rpois(1, N_TOT * apply(df,1,sum))) })
n_susc = sapply(1:B, function(b){rpois(1, (N_TOT-max(nb_people_infectious_at_the_event_all[b],
                                             rpois(1, N_TOT * apply(df,1,sum)))) * 
                                        (1- rbeta(1, beta_immunity_params$a, beta_immunity_params$b))) })
...
#sim=c()
sim<- rbind(sim, 
            data.frame("n" =n_infections_sim , type="All Masks", date=input$date_event)
)
...
library(cowplot)
n_simulated_infections2 = sim
n_simulated_infections2$type = factor(n_simulated_infections2$type , levels=c("No Masks", "Half Masks", "All Masks", "Half Capacity", "Half Duration"))
ggplot(n_simulated_infections2, aes(x = as.factor(type),y=n, fill=as.factor(type)))+ theme_bw() +
...
vac_pred[1,"delta_one_dose"] = as.numeric(vac_df %>% filter(date == max(vac_df$date)) %>% mutate(m=people_vaccinated_per_hundred - people_fully_vaccinated_per_hundred ) %>% dplyr::select(m))
vac_pred[1,"delta_second_dose"] = as.numeric(vac_df %>% filter(date == max(vac_df$date)) %>% mutate(m=people_fully_vaccinated_per_hundred )  %>% dplyr::select(m))
