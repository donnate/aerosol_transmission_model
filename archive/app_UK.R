...
english_prev <- readxl::read_xlsx("covid19infectionsurveydatasets20210319.xlsx", sheet="1b", trim_ws = TRUE, col_types = c("date",rep("numeric",6 ),rep("text",3 )), skip=4)[2:nrow(english_prev),]
english_prev["Region"] = "England"
irish_prev <- readxl::read_xlsx("covid19infectionsurveydatasets20210319.xlsx", sheet="4b", trim_ws = TRUE, col_types = c("date",rep("numeric",6 ),rep("text",3 )), skip=4)[2:nrow(irish_prev),]
irish_prev["Region"] = "Northern Ireland"
wales_prev <- readxl::read_xlsx("covid19infectionsurveydatasets20210319.xlsx", sheet="3b", trim_ws = TRUE, col_types = c("date",rep("numeric",6 ),rep("text",3 )), skip=4)[2:nrow(wales_prev),]
wales_prev["Region"] = "Wales"
scotland_prev <- readxl::read_xlsx("covid19infectionsurveydatasets20210319.xlsx", sheet="5b", trim_ws = TRUE, col_types = c("date",rep("numeric",6 ),rep("text",3 )), skip=4)[2:nrow(scotland_prev),]
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "part",
                  label = "Which part of the UK do you live in?",
                  choices = c("England", "Wales", "Scotland", "Nothern Ireland"),
                  selected="England"),
...
server <- function(input, output, session) {
  ...
  dataInput <- reactive({
    ...
    prevalence_df = compute_prevalence(as.character(input$date_event), data2fit, country="United Kingdom",
    ...
    filtered_prevalence_df = prevalence_df %>%
      dplyr::filter(Date_of_infection<=as.Date(input$date_event),
                    Date_of_infection>as.Date(input$date_event) - 15
      )
    ...
    vaccinations_full_df = compute_vaccinations(as.character(input$date_event), data2fit, country="United Kingdom",
                                       country_data = COUNTRY_DATA,nb_curves=100, 
                                       distance=Difference_function, 
                                       period4predicting=NULL, period4fitting =NULL, 
                                       div=2, distance_type="MSE")
    
    vaccinations_partial_df = compute_vaccinations(as.character(input$date_event), data2fit, country="United Kingdom",
                                                country_data = COUNTRY_DATA,nb_curves=100, 
                                                distance=Difference_function, 
                                                period4predicting=NULL, period4fitting =NULL, 
                                                div=2, distance_type="MSE")
    ...
    group_assignment = sapply(0:(length(SENSITIVITY)),
                              function(x){paste0("p",x)})
    df[sapply(1:(length(SENSITIVITY)), function(x){paste0("p",x)})] =  sapply(1:length(SENSITIVITY),
                                                                              function(i){
                                                                                apply(df, 1, function(x){
                                                                                  compute_infectiousness_probability(sensitivity = SENSITIVITY[16-i],
