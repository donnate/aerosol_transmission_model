...
index = sample(1:nrow(data_with_index), B, replace = TRUE)
n_tot  = data_with_index$Total.Pop.at.Event[index]
nb_people_infectious_at_the_event_all <- data_with_index$Index.Cases[index]
nb_people_infectious_at_the_event <- data_with_index$Index.Cases[index]
...
for (n in c(10, 20, 50, 100, 200, 300, 500, 800, 1000, 2000, 3000, 4000, 5000, 7000, 10000 )){
  region="United Kingdom"
  ...
  #  }))
  n_susc = sapply(1:B, function(b){rpois((N_TOT-max(nb_people_infectious_at_the_event_all[b],
                                                    rpois(1, N_TOT * apply(df,1,sum)))) *
                                           (1- rbeta(N_TOT, beta_immunity_params$a, beta_immunity_params$b))) })
...
library(EnvStats)
alpha = log(5)/log(4)
volumes = data.frame(1^2, 3^2)
colnames(volumes) = c("Close", "Medium")
proba_infection_sim <- sapply(1:B, function(b){
  n = nb_infective_people[b]
  people = n_tot[b] 
  vol  = as.numeric(volumes[ n_density[b]])
...
res_tot = rbind(res_tot,
                data.frame(x=n_infections_sim, "n" = n))
}
